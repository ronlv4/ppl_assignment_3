...
export const parseTraceExp: (params: Sexp[]) => Result<TraceExp> =
    (params) => isString(params[0]) ? makeOk(makeTraceExp(makeVarRef(params[0]))) : makeFailure("invalid")
        // mapv(
        // mapResult((sexp: Sexp) => isString(sexp) ? makeOk(sexp) : makeFailure("invalid"), params),
        // (x:string[]) => makeTraceExp(makeVarRef(x[0]))
        // )
    // (params) => mapv(mapResult((sexp: Sexp):Result<Sexp> => isString(sexp) ? makeOk(sexp) : makeFailure("param is not valid"), params),
    //     (x: Sexp[]) => makeOk(makeTraceExp(makeVarRef(x[0]))))
        // isVarRef(params) ? makeOk(makeTraceExp(params)) : makeFailure("message")
